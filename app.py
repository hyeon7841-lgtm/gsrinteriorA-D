...
if menu == "입고문의 처리":
    ...
    if not st.session_state.vendor:
        ...
        df = pd.read_sql(
            "SELECT * FROM requests WHERE 업체명=? AND 상태!='완료'",
            conn, params=(st.session_state.vendor,)
        )
        st.dataframe(df, hide_index=True)

        if not df.empty:
            선택ID = st.selectbox("문의 선택", df["id"])
...
if menu == "데이터 관리":
    ...
    if not st.session_state.admin:
        ...
        with st.form("map_form"):
            ...
            지역팀 = col2.selectbox("지역팀", 지역팀목록)
            영업팀 = col3.selectbox("영업팀", 영업팀목록)
            업체명 = col4.selectbox("업체명", list(업체계정.keys()))

            if st.form_submit_button("매칭 저장 / 수정"):
                c.execute("""
                ...
                c.execute("""
                INSERT INTO vendor_mapping (부문,지역팀,영업팀,업체명)
                VALUES (?,?,?,?)
                """,(부문,지역팀,영업팀,업체명))

                # 기존 문의 연동 업데이트
                c.execute("""
                UPDATE requests SET 업체명=?
                WHERE 부문=? AND 지역팀=? AND 영업팀=?
                """,(업체명,부문,지역팀,영업팀))

                conn.commit()
                st.success("업체매칭이 저장되었습니다.")
                st.rerun()

        vm = pd.read_sql("SELECT * FROM vendor_mapping ORDER BY 부문,지역팀,영업팀", conn)
        st.dataframe(vm, hide_index=True)

        # ---------- 전체 데이터 ----------
        st.subheader("📋 전체 문의 데이터")
        ...
        line = alt.Chart(g).mark_line(color="red", point=True).encode(
            x="업체명",
